<template>
  ...
  <Modal :show="showDeleteModal" @close="showDeleteModal = false" :maxWidth="'lg'">
    <template v-slot>
      <div class="px-12 py-6">
        <h2 class="text-2xl mb-3 text-[#381D4F]">Hapus Sertifikat</h2>
        <p>Apakah Anda yakin ingin menghapus sertifikat ini?</p>
        <div class="flex justify-end mt-4">
          <button @click="deleteApplicantCertificate" class="px-4 py-2 bg-[#5742F5] text-white rounded-md">Hapus</button>
        </div>
      </div>
    </template>
  </Modal>
